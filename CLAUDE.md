...
`specs/008-authority-limits/plan.md` (Feature: Autoritäts-Grenzen für die Rollen-/Permission-Verwaltung — Privilege-Escalation-Schutz über eine weight-basierte Autoritäts-Achse. `authorityWeight(actor)` = höchstes weight der reachable Rollen (aktive Grants + Vererbung, bewusst NICHT `primaryRoleOf` wg. teamRank); Top-Tier = höchstes weight im System. Regeln: Rollen-Management/-Grant nur unterhalb (non-top strikt `<`, Top-Tier `≤`, nie über Max); Permission-Delegation nur was man selbst hält, Wildcard/`.*` nur mit `*`; Ziel-Autoritäts-Ceiling; Read-Filter (Rollen-Liste weight-gefiltert, Permissions-Tab höher-autorisierter Spieler → 403, Suche/`/me` ungefiltert); ergebnisbasierter Lockout-Schutz (letzter Top-Tier → 409). Durchsetzung: pure `RoleAuthority` (core-domain) + `PermissionAuthorityService` (application) + Guards in `PermissionAdminService` nach `requirePermission` + Read-Gate in `WebPermissionController`; neue Exceptions 403 `authority_ceiling` / 409 `last_top_tier`. KEINE Schema-/Protocol-Änderung. Branch `008-authority-limits`. — Vorher: `specs/007-web-economy-read/plan.md` (Web-Economy Read + SSE unter `/api/web/economy/**`).
<!-- SPECKIT END -->
