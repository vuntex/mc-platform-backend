...
`specs/007-web-economy-read/plan.md` (Feature: Web-Economy Read-Backend + SSE (Slice 1) — read-only Projektions-Pfad fürs Webinterface: web-only Endpunkte unter `/api/web/economy/**` (hinter der `/api/web/**`-JWT-Chain): Sammel-Balances (`GET /api/web/economy/players/{uuid}/balances`), serverweite Transaktions-History (`GET /api/web/economy/history` mit `source`-Filter + Keyset), Transaktions-Detail per `transactionId` (Transfer → zwei Legs), SSE-Bridge auf dem bestehenden `mc:economy:balance`-Channel. Kern: neuer Outbound-Port `EconomyReadStore`, in den `findHistory`+`circulation` 1:1 aus `EconomyEventStore` umziehen (Muster-Leck-Fix; `EconomyHistoryService`+`EconomyStatsService` umgehängt) + neue Reads `playerBalances`/`findServerHistory`/`findTransaction`; geteilte Keyset-Helper; additive `EconomyEventEntry`-Erweiterung (`playerUuid`/`playerName`); kein neuer Codec, `PlatformProtocol.create()` unverändert; neuer Index V16 `idx_event_seq_desc`; Gate `permission.economy.read` (403); 404 `economy_transaction_not_found`; Branch `007-web-economy-read`).
<!-- SPECKIT END -->
